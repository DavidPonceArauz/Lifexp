...
class _HabitsScreenState extends ConsumerState<HabitsScreen>
  ...
  GlobalKey<HabitCompleteAnimationState> _keyFor(int habitId) => _animKeys
      .putIfAbsent(habitId, () => GlobalKey<HabitCompleteAnimationState>());

  @override
  void initState() {
    super.initState();
    _loadReminderPrefs();
    // ── Fix Bug 1: el popup de freeze solo se muestra desde Home ──
    // HabitsScreen ya NO llama _maybeShowFreezePopup() para evitar
    // que aparezca doble. El popup vive únicamente en home_screen.dart.
  }
  ...
  List<dynamic> _applyFilters(List habits, List streakData) {
    ...
    if (_fStatus != 'TODOS') {
      filtered = filtered
          .where((s) => _statusKeyToString(s.statusKey) == _fStatus)
          .toList();
    }
    if (_fCategory != null) {
      final catMap = {for (var h in habits) h.id: h.category};
      filtered =
          filtered.where((s) => catMap[s.habitId] == _fCategory).toList();
    }
    if (_fFreq != 'TODAS') {
    ...
    if (_fSort == 'RACHA') {
      ...
      filtered.sort((a, b) =>
          (catMap[a.habitId] ?? '').compareTo(catMap[b.habitId] ?? ''));
    }
    return filtered;
  }
        ...
        builder: (ctx, setDlg) => AlertDialog(
          ...
          title: Text('❄ USAR FREEZE',
              style: GoogleFonts.pressStart2p(
                  color: AutumnColors.freeze, fontSize: 12)),
          content: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
            Text(
                'Ayer ($yesterdayStr) no completaste estos hábitos.\n'
                        ...
                        onPressed: noFreezes
                            ...
                            : () async {
                                ...
                                if (ok) {
                                  freezesLeft[0]--;
                                  setDlg(() {});
                                  _setMsg('❄ FREEZE aplicado a "${h.name}"',
                                      AutumnColors.freeze);
                                }
                              },
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDlg) => AlertDialog(
          ...
          title: Text('NUEVO HÁBITO',
              style: GoogleFonts.pressStart2p(
                  color: AutumnColors.mossGreen, fontSize: 12)),
          content: SizedBox(
              width: 340,
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Autocomplete<String>(
                  ...
                  fieldViewBuilder: (ctx2, ctrl2, fn, _) => TextField(
                    ...
                    focusNode: fn,
                    onChanged: (v) {
                      if (v.isEmpty) setDlg(() => selectedCategory = null);
                    },
                    style: GoogleFonts.pressStart2p(
                    ...
                    decoration: InputDecoration(
                        ...
                        prefixIcon: selectedCategory != null
                            ? Padding(
                                ...
                                child:
                                    Text(_categories.firstWhere((cat) => cat['label'] == selectedCategory, orElse: () => {'emoji': '📁', 'label': ''})['emoji']!,
                                        style: const TextStyle(fontSize: 16)))
                            : const Icon(Icons.folder_outlined,
                                size: 16, color: AutumnColors.mossGreen),
                        ...
                        focusedBorder: OutlineInputBorder(
                            ...
                            borderSide: const BorderSide(
                                color: AutumnColors.mossGreen, width: 1.5)),
          ...
          actions: [
            ...
            ElevatedButton(
                style: ElevatedButton.styleFrom(
                    backgroundColor: AutumnColors.mossGreen),
                onPressed: () async {
                  Navigator.pop(ctx);
                  await ref
                      .read(habitsProvider.notifier)
                      .createHabit(nameCtrl.text, selectedCategory ?? '');
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDlg) => AlertDialog(
          ...
          title: Text('SELECCIONAR FECHA',
              style: GoogleFonts.pressStart2p(
                  color: AutumnColors.mossGreen, fontSize: 11)),
          content: SizedBox(
              width: 300,
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                Row(children: [
                  IconButton(
                      ...
                      onPressed: () => setDlg(() => calDate =
                          DateTime(calDate.year, calDate.month - 1, 1))),
                  Expanded(
                      child: Text(
                          DateFormat('MMMM yyyy').format(calDate).toUpperCase(),
                          style: GoogleFonts.pressStart2p(
  ...
  Widget _buildCalendarGrid(
      DateTime calDate, DateTime selectedDate, BuildContext ctx) {
    final c = ctx.ac;
    final today = DateTime.now();
    final daysInMonth = DateTime(calDate.year, calDate.month + 1, 0).day;
    final startOffset =
        (DateTime(calDate.year, calDate.month, 1).weekday - 1) % 7;
    final cells = <Widget>[];
      ...
      cells.add(GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
              ...
              border: Border.all(
                  color: isSelected ? AutumnColors.mossGreen : c.divider)),
          child: Center(
              child: Text('$d',
                  style: GoogleFonts.pressStart2p(
                      fontSize: 8,
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('HABITS 🔥',
              style: GoogleFonts.pressStart2p(
                  fontSize: 14, color: AutumnColors.mossGreen)),
        ...
        actions: [
          IconButton(
              icon: const Icon(Icons.add_circle_outline,
                  color: AutumnColors.mossGreen, size: 26),
              tooltip: 'Nuevo hábito',
              onPressed: _openCreateHabitPopup),
        ],
      ...
      body: isLoading && habits.isEmpty
          ? const Center(
              child: CircularProgressIndicator(color: AutumnColors.mossGreen))
          : RefreshIndicator(
              ...
              child: ListView(padding: const EdgeInsets.all(16), children: [
                ...
                const SizedBox(height: 10),
                _buildHabitFilterPanel(
                    context, habits, allStreaks, filteredStreaks),
                const SizedBox(height: 10),
                if (allStreaks.isNotEmpty &&
                    filteredStreaks.length != allStreaks.length)
                  Padding(
                      padding: const EdgeInsets.only(bottom: 6),
                      child: Text(
                          'Mostrando ${filteredStreaks.length} de ${allStreaks.length} hábitos',
                          style: GoogleFonts.pressStart2p(
                ...
                Builder(builder: (ctx) {
                  final filteredIds =
                      filteredStreaks.map((s) => s.habitId as int).toSet();
                  final visibleHabits = (_fStatus == 'TODOS' &&
                          _fCategory == null &&
                          _fFreq == 'TODAS')
                      ? habits
                      : habits
                          .where((h) => filteredIds.contains(h.id))
                          .toList();

                  if (visibleHabits.isEmpty) {
                    return PixelEmptyState(
                      type: EmptyStateType.habits,
                      customTitle: habits.isEmpty
                          ? 'SIN HÁBITOS\nAÚN'
                          : 'SIN\nRESULTADOS',
                      customSubtitle: habits.isEmpty
                          ...
                          : 'Intenta cambiar\nlos filtros.',
                      customActionLabel:
                          habits.isEmpty ? '+ NUEVO HÁBITO' : null,
  ...
  Widget _buildHabitCard(dynamic habit, dynamic state) {
    ...
    final accentColor =
        isCompleted ? AutumnColors.mossGreen : AutumnColors.accentOrange;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Icon(Icons.local_fire_department,
              color: AutumnColors.accentGold, size: 16),
        ...
        if (streakData.isEmpty)
          Text('NO ACTIVE HABITS',
              style:
                  GoogleFonts.pressStart2p(fontSize: 9, color: c.textDisabled))
        else
          ...streakData.map((s) {
              ...
              child: Row(children: [
                ...
                SizedBox(
                    ...
                    child: Row(children: [
                      if (streak > 0) ...[
                        const Icon(Icons.local_fire_department,
                            color: AutumnColors.accentGold, size: 10),
                        const SizedBox(width: 2),
                        Text('${streak}d',
                            style: GoogleFonts.pressStart2p(
                                fontSize: 7, color: AutumnColors.accentGold)),
                      ] else
                        Text('—',
                            style: GoogleFonts.pressStart2p(
                                fontSize: 7, color: c.textDisabled)),
                ...
                SizedBox(
                    ...
                    child: Row(children: [
                      Icon(statusIcons[statusKey] ?? Icons.schedule,
                          color: statusColors[statusKey], size: 10),
                      const SizedBox(width: 2),
                      Text(statusLabels[statusKey] ?? '',
                          style: GoogleFonts.pressStart2p(
                              fontSize: 7, color: statusColors[statusKey])),
                    ])),
                SizedBox(
                    width: 52,
                    child: Row(children: [
                      const Icon(Icons.ac_unit,
                          color: AutumnColors.freeze, size: 10),
                      const SizedBox(width: 2),
                      Text(daysToFreeze > 0 ? 'en ${daysToFreeze}d' : '¡HOY!',
                          style: GoogleFonts.pressStart2p(
                              fontSize: 7, color: AutumnColors.freeze)),
                    ])),
              ]),
            );
          }),
  ...
  Widget _buildHabitFilterPanel(BuildContext context, List habits,
      List allStreaks, List filteredStreaks) {
    final c = context.ac;
              ...
              ? Padding(
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        if (cats.isNotEmpty) ...[
                          const SizedBox(height: 10),
                          _hfLabel(c, 'CATEGORÍA'),
                          const SizedBox(height: 6),
                          Wrap(spacing: 5, runSpacing: 5, children: [
                            GestureDetector(
                                onTap: () => setState(() => _fCategory = null),
                                child: AnimatedContainer(
                                    duration: const Duration(milliseconds: 130),
                                    padding: const EdgeInsets.symmetric(
                                        horizontal: 8, vertical: 5),
                                    decoration: BoxDecoration(
                                        color: _fCategory == null
                                            ? AutumnColors.mossGreen
                                            : c.bgSurface,
                                        borderRadius: BorderRadius.circular(8),
                                        border: Border.all(
                        ...
                        Divider(color: c.divider, height: 14),
                        _hfLabel(c, 'ORDENAR POR'),
                        const SizedBox(height: 6),
                        Row(
                            children: [
                          {
                            'v': 'RACHA',
                            'i': Icons.local_fire_department_rounded
                          },
  ...
  Widget _hfLabel(dynamic c, String t) => Text(t,
