...
class _TodoS {
  final bool isEs;
  const _TodoS(this.isEs);

  String get title            => 'SIDE QUESTS';
  String get subtitle         => isEs ? 'Arrastra - Desliza - Toca' : 'Drag - Swipe - Tap';

  String get colPending       => isEs ? 'PEND.'   : 'PEND.';
  String get colProgress      => isEs ? 'PROC.'   : 'PROG.';
  String get colDone          => isEs ? 'HECHO'   : 'DONE';
  String get empty            => isEs ? 'vacio'   : 'empty';

  String get newTask          => isEs ? 'NUEVA TAREA'             : 'NEW TASK';
  String get editTask         => isEs ? 'EDITAR TAREA'            : 'EDIT TASK';
  String get name             => isEs ? 'NOMBRE'                  : 'NAME';
  String get namePlaceholder  => isEs ? 'Nombre de la tarea...'   : 'Task name...';
  String get priority         => isEs ? 'PRIORIDAD'               : 'PRIORITY';
  String get deadline         => isEs ? 'FECHA LIMITE'            : 'DEADLINE';
  String get noDeadline       => isEs ? 'Sin fecha limite'        : 'No deadline';
  ...
  String get create           => isEs ? 'CREAR'                   : 'CREATE';

  String get high             => isEs ? 'ALTA'    : 'HIGH';
  String get medium           => isEs ? 'MEDIA'   : 'MED';
  String get low              => isEs ? 'BAJA'    : 'LOW';
  String get all              => isEs ? 'TODAS'   : 'ALL';

  String get descLabel        => isEs ? 'DESCRIPCION'   : 'DESCRIPTION';
  String get dateLabel        => isEs ? 'FECHA LIMITE'  : 'DEADLINE';
  String get noDate           => isEs ? 'Sin fecha'     : 'No date';
  String get createdLabel     => isEs ? 'CREADA'        : 'CREATED';
  String get edit             => isEs ? 'EDITAR'        : 'EDIT';
  String get delete           => isEs ? 'ELIMINAR'      : 'DELETE';

  String get statusPending    => isEs ? 'PENDIENTE'  : 'PENDING';
  String get statusInProgress => isEs ? 'EN PROCESO' : 'IN PROGRESS';
  String get statusDone       => isEs ? 'HECHA'      : 'DONE';

  String get swipeEdit        => isEs ? 'Editar'      : 'Edit';
  String get swipeDetail      => isEs ? 'Ver detalle' : 'Detail';
  String get swipeMove        => isEs ? 'Mover'       : 'Move';
  ...
  String get sortPriority     => isEs ? 'PRIORIDAD'   : 'PRIORITY';
  String get categorySearch   => isEs ? 'Escribe para buscar categoria...' : 'Type to search category...';

  String late_(int d)  => isEs ? 'ATRASADA ${d}d' : 'LATE ${d}d';
...
class _TodoScreenState extends ConsumerState<TodoScreen>
  ...
  Map<String, dynamic> _deadlineInfo(String? deadline, _TodoS s) {
    ...
    try {
      ...
      if (diff < 0)  return {'text': s.late_(diff.abs()), 'color': AutumnColors.accentRed};
      if (diff == 0) return {'text': s.today,             'color': AutumnColors.accentRed};
      if (diff == 1) return {'text': s.tomorrow,          'color': AutumnColors.accentGold};
  ...
  void _openTodoDialog({Todo? todo}) {
    ...
    final s      = _TodoS(lang == AppLanguage.es);
    ...
    final priorityLabels = {3: s.high, 2: s.medium, 1: s.low};
    final priorityValues = {s.high: 3, s.medium: 2, s.low: 1};
    final priorityColors = {
      s.high:   AutumnColors.accentRed,
      s.medium: AutumnColors.accentGold,
      s.low:    AutumnColors.mossGreen,
    };

    String  priority         = isEdit ? (priorityLabels[todo.priority] ?? s.medium) : s.medium;
        ...
        child: SizedBox(width: 400, height: 580, child: Column(children: [
          Container(
              ...
              child: Row(children: [
                ...
                Text(isEdit ? s.editTask : s.newTask,
          ...
          Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(20),
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                TextField(controller: titleCtrl,
                    ...
                    decoration: _deco(c, s.namePlaceholder)),
                const SizedBox(height: 14),
                _lbl(c, s.priority),
                Row(children: [s.high, s.medium, s.low].map((p) {
                  final sel = p == priority;
                  return Expanded(child: GestureDetector(
                      onTap: () => setDlg(() => priority = p),
                      child: AnimatedContainer(duration: const Duration(milliseconds: 150),
                          margin: EdgeInsets.only(right: p != s.low ? 6 : 0),
  ...
  void _openDetailDialog(Todo todo) {
    ...
    final di = _deadlineInfo(todo.deadline, s);
    final statusLabels = {
  ...
  Widget _buildColumn(BuildContext context, String title, String status,
      Color color, List<Todo> items, _TodoS s) {
    final c = context.ac;
          ...
          child: Column(children: [
            ...
            Expanded(child: items.isEmpty
              ...
              Text(s.empty, style: GoogleFonts.pressStart2p(fontSize: 7, color: color.withValues(alpha: 0.35))),
                ...
                itemBuilder: (_, i) => _buildTodoCard(context, items[i], color, s))),
          ]),
        );
      },
    ));
  ...
  Widget _buildTodoCard(BuildContext context, Todo todo, Color accentColor, _TodoS s) {
    final c              = context.ac;
    final di             = _deadlineInfo(todo.deadline, s);
    final priorityLabels = {3: s.high, 2: s.medium, 1: s.low};
    ...
    return Dismissible(
      ...
      child: Draggable<Todo>(
        ...
        child: GestureDetector(onTap: () => _openDetailDialog(todo),
            child: _cardContent(context, todo, di, priorityLabels)),
      ),
    );
  }
  ...
  Widget _buildFilterPanel(BuildContext context, TodosState state) {
    ...
    final priorityMap = {s.all: 'TODAS', s.high: 'ALTA', s.medium: 'MEDIA', s.low: 'BAJA'};
    final sortMap     = {s.sortDate: 'FECHA', s.sortPriority: 'PRIORIDAD'};
        ...
        AnimatedSize(duration: const Duration(milliseconds: 220), curve: Curves.easeInOut,
          child: _filtersOpen
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(s.priority, style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textDisabled)),
                const SizedBox(height: 6),
                Row(children: [s.all, s.high, s.medium, s.low].map((o) {
                  final internalVal = priorityMap[o] ?? o;
                  final sel = internalVal == state.filterPriority;
                  const pc = {
                    'TODAS': AutumnColors.accentOrange, 'ALTA': AutumnColors.accentRed,
                    'MEDIA': AutumnColors.accentGold,   'BAJA': AutumnColors.mossGreen,
                  };
                  final col = pc[internalVal] ?? AutumnColors.accentOrange;
                  return Expanded(child: GestureDetector(
                      onTap: () => ref.read(todosProvider.notifier).setFilterPriority(internalVal),
                ...
                Text(s.sortBy, style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textDisabled)),
                const SizedBox(height: 6),
                Row(children: [
                  {'v': s.sortDate,     'i': Icons.calendar_today_rounded},
                  {'v': s.sortPriority, 'i': Icons.flag_rounded},
                ].map((item) {
                  final v         = item['v'] as String;
                  final ico       = item['i'] as IconData;
                  final internalV = sortMap[v] ?? v;
                  final sel       = state.filterSort == internalV;
                  return Expanded(child: GestureDetector(
                      onTap: () => ref.read(todosProvider.notifier).setFilterSort(internalV),
                      child: AnimatedContainer(duration: const Duration(milliseconds: 130),
                          margin: const EdgeInsets.only(right: 5),
                ...
                if (cats.isNotEmpty) ...[
                  const SizedBox(height: 10),
                  Text(s.category, style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textDisabled)),
                  const SizedBox(height: 6),
  ...
  Widget _catAutocomplete(dynamic c, _TodoS s, {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.add_circle_outline, color: AutumnColors.accentOrange, size: 24),
              tooltip: s.newTask, onPressed: () => _openTodoDialog()),
        ],
      ...
      body: state.isLoading && state.allTodos.isEmpty
        ...
        Expanded(child: state.allTodos.isEmpty
            ...
            child: Row(children: [
              _buildColumn(context, s.colPending,  'pending',     AutumnColors.accentOrange, state.pending,    s),
              const SizedBox(width: 6),
              _buildColumn(context, s.colProgress, 'in_progress', AutumnColors.accentGold,   state.inProgress, s),
              const SizedBox(width: 6),
              _buildColumn(context, s.colDone,     'done',        AutumnColors.mossGreen,    state.done,       s),
            ]))),
      ]),
    );
  }
