...
class LanguageNotifier extends StateNotifier<AppLanguage> {
  LanguageNotifier() : super(AppLanguage.es) { _load(); }
...
class _S {
  ...
  String get sectionDanger       => isEs ? '⚠️   ZONA DE PELIGRO'      : '⚠️   DANGER ZONE';
  ...
  String get resetXpSub          => isEs ? 'Vuelve al nivel 1 con 0 XP': 'Back to level 1 with 0 XP';
  String get deleteAccount       => isEs ? 'BORRAR CUENTA'            : 'DELETE ACCOUNT';
  String get deleteAccountSub    => isEs ? 'Elimina tu cuenta permanentemente' : 'Permanently delete your account';
  String get danger              => isEs ? 'ZONA DE PELIGRO'          : 'DANGER ZONE';
  String get typeConfirm         => isEs ? 'Escribe CONFIRMAR para continuar' : 'Type CONFIRM to continue';
  ...
  String get rateAppSub          => isEs ? 'Play Store / App Store'   : 'Play Store / App Store';
  ...
  String get logoutQ             => isEs ? '¿Seguro que quieres salir?' : 'Are you sure you want to log out?';
  String get logout2             => isEs ? 'SALIR'                    : 'LEAVE';
  String get loadingProfile      => isEs ? 'CARGANDO PERFIL...'       : 'LOADING PROFILE...';
...
class _ProfileScreenState extends ConsumerState<ProfileScreen>
  ...
  void initState() {
    ...
    _xpBarCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 1200));
    _xpBarAnim = CurvedAnimation(parent: _xpBarCtrl, curve: Curves.easeOutCubic);
    _loadAll();
  }
  ...
  Future<void> _loadAll() async {
    ...
    if (mounted) { setState(() => _loading = false); _xpBarCtrl.forward(); }
  }

  Future<void> _loadProfile() async {
    try {
      final profile = await _db.from('profiles')
          .select('username, email, total_xp').eq('id', widget.userId).single();
      ...
      final currentXp    = (totalXp - xpForCurrent).clamp(0, xpForNext - xpForCurrent);
      if (mounted) setState(() {
  ...
  Future<void> _loadStats() async {
    try {
      final results = await Future.wait([
        _db.from('habit_logs').select('id').eq('user_id', widget.userId).eq('completed', true),
        _db.from('goals').select('id').eq('user_id', widget.userId).eq('status', 'completed'),
        _db.from('todos').select('id').eq('user_id', widget.userId).eq('status', 'done'),
        _db.from('habit_logs').select('date').eq('user_id', widget.userId).eq('completed', true).order('date'),
      ]);
      ...
      for (final d in dates) {
        if (prev == null || d.difference(prev).inDays == 1) { currentStreak++; }
        else if (d.difference(prev).inDays > 1)             { currentStreak = 1; }
    ...
    } catch (e) { debugPrint('loadStats error: $e'); }
  }
    ...
    final confirm = await showDialog<bool>(context: context,
      builder: (ctx) => AlertDialog(
        ...
        title: Text(s.logout.toUpperCase(),
            style: GoogleFonts.pressStart2p(fontSize: 10, color: AutumnColors.accentRed)),
        content: Text(s.logoutQ,
            style: GoogleFonts.pressStart2p(fontSize: 8, color: c.textSecondary)),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false),
              child: Text(s.cancel, style: GoogleFonts.pressStart2p(fontSize: 8, color: c.textDisabled))),
          TextButton(onPressed: () => Navigator.pop(ctx, true),
              child: Text(s.logout2, style: GoogleFonts.pressStart2p(fontSize: 8, color: AutumnColors.accentRed))),
        ],
      ),
    );
  ...
  Future<void> _resetXp() async {
    ...
    try {
      await _db.from('profiles').update({'total_xp': 0}).eq('id', widget.userId);
      setState(() { _totalXp = 0; _level = 1; _currentXp = 0; _xpForNext = 500; });
      _xpBarCtrl.reset(); _xpBarCtrl.forward();
      HapticFeedback.heavyImpact();
      if (mounted) _showSnack(s.isEs ? 'XP reseteado a 0' : 'XP reset to 0', AutumnColors.accentOrange);
    } catch (e) { debugPrint('resetXp error: $e'); }
  }
  ...
  Future<void> _deleteAccount() async {
    ...
    try {
      ...
      if (mounted) _showSnack(
        s.isEs ? 'Error al borrar cuenta. Intenta de nuevo.' : 'Error deleting account. Try again.',
  ...
  Future<bool> _showDangerDialog({
    ...
    required String confirmWord, required Color color,
  }) async {
    final c = context.ac;
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDlg) => AlertDialog(
          ...
          title: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(title, style: GoogleFonts.pressStart2p(fontSize: 9, color: c.textPrimary)),
          ]),
          content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(message, style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textSecondary, height: 1.8)),
            ...
            TextField(
              ...
              onChanged: (v) => setDlg(() => isValid = v.toUpperCase() == confirmWord.toUpperCase()),
            ),
          ]),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false),
  ...
  void _showSnack(String msg, Color color) {
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(msg, style: GoogleFonts.pressStart2p(fontSize: 7, color: Colors.white)),
        ...
        builder: (_, scrollCtrl) => Container(
          ...
          child: Column(children: [
            ...
            Padding(padding: const EdgeInsets.symmetric(horizontal: 20),
                child: Text(title, style: GoogleFonts.pressStart2p(fontSize: 10, color: AutumnColors.accentOrange))),
            const SizedBox(height: 4),
            Divider(color: c.divider),
            Expanded(child: SingleChildScrollView(
              controller: scrollCtrl,
              padding: const EdgeInsets.fromLTRB(20, 8, 20, 32),
              child: Text(body, style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textSecondary, height: 2.2)),
            )),
          ]),
        ),
      ),
  ...
  void _openPrivacy() {
    final s = _S(ref.read(languageProvider));
    _showLegalSheet(s.privacy, s.isEs
  ...
  void _openTerms() {
    final s = _S(ref.read(languageProvider));
    _showLegalSheet(s.terms, s.isEs
        ? 'TÉRMINOS DE USO\n\nVersión 1.0 — Enero 2025\n\n'
  ...
  Future<void> _openContact() async {
    final uri = Uri(scheme: 'mailto', path: 'support@lifexp.app', query: 'subject=LifeXP Support');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _loading ? _buildLoading(c, s) : CustomScrollView(slivers: [
        _buildAppBar(c),
        SliverToBoxAdapter(
          child: AnimatedBuilder(
            ...
            builder: (_, child) => FadeTransition(
              ...
              child: Transform.translate(
                  offset: Offset(0, 20 * (1 - _entryCtrl.value)), child: child),
            ),
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                // Logros
                _buildSection(c: c, label: s.sectionAchievements,
                    accent: AutumnColors.accentGold,
                    child: _buildBadgesGrid(c, s)),
                const SizedBox(height: 8),

                // Estadísticas
                _buildSection(c: c, label: s.sectionStats,
                    accent: AutumnColors.accentOrange,
                    child: _buildStatsGrid(c, s)),
                const SizedBox(height: 8),

                // Gráficas
                _buildSection(c: c, label: s.sectionCharts,
                ...
                // Apariencia
                _buildSection(c: c, label: s.sectionAppearance,
                    accent: AutumnColors.accentGold,
                    child: _buildAppearanceContent(c, s)),
                const SizedBox(height: 8),

                // Información
                _buildSection(c: c, label: s.sectionInfo,
                    accent: AutumnColors.mossGreen,
                ...
                // Zona de peligro
                _buildSection(c: c, label: s.sectionDanger,
  ...
  Widget _buildSection({
    required dynamic c, required String label, required Color accent,
    required Widget child, bool initiallyExpanded = true,
  }) {
    return Container(
      decoration: BoxDecoration(
  ...
  Widget _buildAppBar(dynamic c) => SliverAppBar(
    backgroundColor: c.bgCard, pinned: true, elevation: 0, automaticallyImplyLeading: false,
    leading: IconButton(
    ...
    title: Row(children: [
      ...
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        Text(_username.toUpperCase(),
            style: GoogleFonts.pressStart2p(fontSize: 10, color: c.textPrimary),
  ...
  Widget _buildXpCard(dynamic c, _S s) {
    final xpPct = _xpForNext > 0 ? (_currentXp / _xpForNext).clamp(0.0, 1.0) : 0.0;
    return Container(
      ...
      child: Column(children: [
        Row(children: [
          Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
              ...
              child: Text('LVL $_level',
                  style: GoogleFonts.pressStart2p(fontSize: 12, color: Colors.white, fontWeight: FontWeight.bold))),
        ...
        ClipRRect(borderRadius: BorderRadius.circular(6), child: Stack(children: [
          ...
          AnimatedBuilder(animation: _xpBarAnim, builder: (_, __) =>
              FractionallySizedBox(widthFactor: xpPct * _xpBarAnim.value,
          ...
          SizedBox(height: 16, child: Center(child: Text('${(xpPct * 100).round()}%',
              style: GoogleFonts.pressStart2p(fontSize: 7,
                  color: xpPct > 0.3 ? Colors.white : c.textDisabled)))),
        ])),
      ]),
    );
  }
      ...
      Container(margin: const EdgeInsets.only(bottom: 12),
          ...
          child: Row(children: [
            Text('$unlocked / ${badges.length}',
                style: GoogleFonts.pressStart2p(fontSize: 11, color: AutumnColors.accentGold, fontWeight: FontWeight.bold)),
            const SizedBox(width: 10),
            Expanded(child: Text(
  ...
  List<_Badge> _buildBadgeList(_S s) => [
    _Badge('🔥', s.isEs ? 'INICIADO'     : 'BEGINNER',    s.isEs ? 'Completa 1 hábito'    : 'Complete 1 habit',     _habitsCompleted >= 1),
    _Badge('⚡', s.isEs ? 'CONSTANTE'    : 'CONSISTENT',  s.isEs ? 'Completa 10 hábitos'  : 'Complete 10 habits',   _habitsCompleted >= 10),
    _Badge('💪', s.isEs ? 'DISCIPLINADO' : 'DISCIPLINED', s.isEs ? 'Completa 50 hábitos'  : 'Complete 50 habits',   _habitsCompleted >= 50),
    _Badge('🌟', s.isEs ? 'IMPARABLE'    : 'UNSTOPPABLE', s.isEs ? 'Completa 100 hábitos' : 'Complete 100 habits',  _habitsCompleted >= 100),
    _Badge('🎯', s.isEs ? 'MISIONERO'    : 'MISSION',     s.isEs ? 'Completa 1 meta'      : 'Complete 1 goal',      _goalsCompleted >= 1),
    _Badge('🏆', s.isEs ? 'CONQUISTADOR' : 'CONQUEROR',   s.isEs ? 'Completa 5 metas'     : 'Complete 5 goals',     _goalsCompleted >= 5),
    _Badge('📋', s.isEs ? 'ORDENADO'     : 'ORGANIZED',   s.isEs ? 'Completa 10 tareas'   : 'Complete 10 tasks',    _todosCompleted >= 10),
    _Badge('✅', s.isEs ? 'PRODUCTIVO'   : 'PRODUCTIVE',  s.isEs ? 'Completa 50 tareas'   : 'Complete 50 tasks',    _todosCompleted >= 50),
    _Badge('🔄', s.isEs ? 'RACHA ×7'     : 'STREAK ×7',   s.isEs ? 'Racha de 7 días'      : '7-day streak',         _maxStreak >= 7),
    _Badge('📅', s.isEs ? 'RACHA ×30'    : 'STREAK ×30',  s.isEs ? 'Racha de 30 días'     : '30-day streak',        _maxStreak >= 30),
    _Badge('🌱', s.isEs ? 'EXPLORADOR'   : 'EXPLORER',    s.isEs ? 'Activo 7 días'        : 'Active 7 days',        _totalDaysActive >= 7),
    _Badge('🌳', s.isEs ? 'VETERANO'     : 'VETERAN',     s.isEs ? 'Activo 30 días'       : 'Active 30 days',       _totalDaysActive >= 30),
  ];
  ...
  Widget _buildBadgeCard(dynamic c, _Badge b) {
    return AnimatedContainer(duration: const Duration(milliseconds: 300),
        ...
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Text(b.name, style: GoogleFonts.pressStart2p(fontSize: 6,
          ...
          const SizedBox(height: 3),
          Text(b.description, style: GoogleFonts.pressStart2p(fontSize: 5, color: c.textDisabled),
              textAlign: TextAlign.center, maxLines: 2, overflow: TextOverflow.ellipsis),
          if (b.unlocked) ...[
            const SizedBox(height: 4),
  ...
  Widget _buildStatsGrid(dynamic c, _S s) {
    final items = [
      ('🔥', '$_habitsCompleted', s.isEs ? 'HÁBITOS\nCOMPLETADOS' : 'HABITS\nCOMPLETED',  AutumnColors.accentOrange),
      ('🎯', '$_goalsCompleted',  s.isEs ? 'METAS\nLOGRADAS'      : 'GOALS\nACHIEVED',    AutumnColors.accentGold),
      ('✅', '$_todosCompleted',  s.isEs ? 'TAREAS\nHECHAS'       : 'TASKS\nDONE',        AutumnColors.mossGreen),
      ('⚡', '$_maxStreak',       s.isEs ? 'RACHA\nMÁX.'          : 'MAX\nSTREAK',        AutumnColors.freeze),
    ];
    return GridView.count(crossAxisCount: 4, shrinkWrap: true,
        physics: const NeverScrollableScrollPhysics(),
        crossAxisSpacing: 8, mainAxisSpacing: 8, childAspectRatio: 0.75,
        children: items.map((st) => Container(
          ...
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            const SizedBox(height: 4),
            Text(st.$2, style: GoogleFonts.pressStart2p(
                fontSize: 13, color: st.$4 as Color, fontWeight: FontWeight.bold)),
            const SizedBox(height: 3),
            Text(st.$3, style: GoogleFonts.pressStart2p(fontSize: 5, color: c.textDisabled),
  ...
  Widget _buildAppearanceContent(dynamic c, _S s) {
    ...
    return Column(children: [
      ...
      Row(children: [
        ...
        const SizedBox(width: 12),
        Expanded(child: Text(s.theme,
            style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textPrimary))),
        // Toggle visual
        GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            child: Row(children: [
              Expanded(child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                    color: !isDark ? AutumnColors.accentGold : Colors.transparent,
                    borderRadius: BorderRadius.circular(7)),
                child: Center(child: Text(s.themeLight, style: GoogleFonts.pressStart2p(
                    fontSize: 6,
              ...
              Expanded(child: AnimatedContainer(
                ...
                decoration: BoxDecoration(
                    color: isDark ? AutumnColors.accentGold : Colors.transparent,
                    borderRadius: BorderRadius.circular(7)),
                child: Center(child: Text(s.themeDark, style: GoogleFonts.pressStart2p(
                    fontSize: 6,
                    color: isDark ? Colors.white : c.textDisabled,
                    fontWeight: isDark ? FontWeight.bold : FontWeight.normal))),
              )),
      ...
      Row(children: [
        ...
        GestureDetector(
          onTap: () {
            final next = lang == AppLanguage.es ? AppLanguage.en : AppLanguage.es;
  ...
  Widget _buildInfoContent(dynamic c, _S s) {
    return Column(children: [
      _infoRow(c, icon: Icons.info_outline_rounded, iconColor: AutumnColors.mossGreen,
          title: s.version, subtitle: '1.0.0'),
      Divider(height: 1, color: c.divider),
      _infoRow(c, icon: Icons.shield_outlined, iconColor: AutumnColors.accentOrange,
          title: s.privacy, onTap: _openPrivacy),
      Divider(height: 1, color: c.divider),
      _infoRow(c, icon: Icons.gavel_rounded, iconColor: AutumnColors.accentOrange,
          title: s.terms, onTap: _openTerms),
      Divider(height: 1, color: c.divider),
      _infoRow(c, icon: Icons.code_rounded, iconColor: AutumnColors.mossGreen,
          title: s.licenses, onTap: _openLicenses),
      Divider(height: 1, color: c.divider),
      _infoRow(c, icon: Icons.mail_outline_rounded, iconColor: AutumnColors.accentGold,
          title: s.contact, subtitle: s.contactSub, onTap: _openContact),
      Divider(height: 1, color: c.divider),
      _infoRow(c, icon: Icons.star_outline_rounded, iconColor: AutumnColors.accentGold,
          title: s.rateApp, subtitle: s.rateAppSub, onTap: _rateApp),
    ]);
  }

  Widget _infoRow(dynamic c, {
    ...
    return InkWell(
      ...
      child: Padding(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(title, style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textPrimary)),
            if (subtitle != null && subtitle.isNotEmpty) ...[
              const SizedBox(height: 3),
              Text(subtitle, style: GoogleFonts.pressStart2p(fontSize: 6, color: c.textDisabled)),
            ],
          ])),
          if (onTap != null)
  ...
  Widget _buildDangerContent(dynamic c, _S s) {
    return Column(children: [
      InkWell(onTap: _resetXp, child: Padding(
        padding: const EdgeInsets.symmetric(vertical: 11),
        child: Row(children: [
          Container(width: 32, height: 32,
              decoration: BoxDecoration(
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(s.resetXp, style: GoogleFonts.pressStart2p(fontSize: 7, color: AutumnColors.accentOrange)),
            const SizedBox(height: 3),
      ...
      InkWell(onTap: _deleteAccount, child: Padding(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 12),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(s.deleteAccount, style: GoogleFonts.pressStart2p(fontSize: 7, color: AutumnColors.accentRed)),
            const SizedBox(height: 3),
            Text(s.deleteAccountSub, style: GoogleFonts.pressStart2p(fontSize: 6, color: c.textDisabled)),
          ])),
          const Icon(Icons.chevron_right_rounded, color: AutumnColors.accentRed, size: 18),
        ]),
      )),
    ]);
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(width: 10),
          Text(s.logout, style: GoogleFonts.pressStart2p(fontSize: 9, color: AutumnColors.accentRed)),
        ]),
      ),
    );
  }
