...
class _HomeS {
  final bool isEs;
  const _HomeS(this.isEs);

  // Secciones
  String get playerStatus    => isEs ? 'ESTADO DEL JUGADOR'    : 'PLAYER STATUS';
  String get upcomingDue     => isEs ? 'PRÓXIMOS VENCIMIENTOS' : 'UPCOMING DEADLINES';
  String get calendar        => isEs ? 'CALENDARIO'            : 'CALENDAR';

  // Tree hero
  String levelTitle(int level) {
    if (level >= 11) return isEs ? 'ÁRBOL LEGENDARIO'     : 'LEGENDARY TREE';
    if (level >= 7)  return isEs ? 'ÁRBOL MADURO'         : 'MATURE TREE';
    if (level >= 3)  return isEs ? 'ÁRBOL EN CRECIMIENTO' : 'GROWING TREE';
    return                  isEs ? 'ÁRBOL JOVEN'          : 'YOUNG TREE';
  }

  // XP hints
  String xpHint(int pct) {
    if (pct >= 80) return isEs ? '¡Casi en el siguiente nivel!' : 'Almost at next level!';
    if (pct >= 50) return isEs ? 'Vas a la mitad del nivel'     : 'Halfway through the level';
    if (pct >= 25) return isEs ? 'Buen comienzo, sigue así'     : 'Good start, keep going';
    return                isEs ? 'Completa hábitos para crecer' : 'Complete habits to grow';
  }

  // Stats grid labels
  String get statGoals       => isEs ? 'METAS'   : 'GOALS';
  String get statStreak      => isEs ? 'RACHA'   : 'STREAK';
  String get statTasks       => isEs ? 'TAREAS'  : 'TASKS';

  // Deadline card
  String get noDeadlines     => isEs ? 'Sin vencimientos próximos' : 'No upcoming deadlines';
  String get overdue         => isEs ? 'VENCIDA'  : 'OVERDUE';
  String get today           => isEs ? 'HOY'      : 'TODAY';
  String get tomorrow        => isEs ? 'MAÑANA'   : 'TOMORROW';

  // Level up banner
  String levelUp(int l)      => isEs ? '¡SUBISTE AL NIVEL $l!'  : 'YOU REACHED LEVEL $l!';
  String get levelUpSub      => isEs ? '¡Sigue así, tu árbol crece!' : 'Keep it up, your tree grows!';

  // Freeze popup
  String get useFreeze       => isEs ? '❄ USAR FREEZE'  : '❄ USE FREEZE';
  String freezeMissed(String day) => isEs
      ? 'Ayer ($day) fallaste estos hábitos.'
      : 'Yesterday ($day) you missed these habits.';
  String freezeAvailable(int n) => isEs
      ? 'Tienes $n freeze${n != 1 ? "s" : ""} disponible${n != 1 ? "s" : ""}.'
      : 'You have $n freeze${n != 1 ? "s" : ""} available.';
  String get noFreezes       => isEs ? 'SIN FREEZES'     : 'NO FREEZES';
  String get goToHabits      => isEs ? '→ IR A HÁBITOS'  : '→ GO TO HABITS';
  String get decideLater     => isEs ? 'DECIDIR DESPUÉS' : 'DECIDE LATER';

  // Calendar legend
  String get legendGoal      => isEs ? 'Meta'     : 'Goal';
  String get legendTask      => isEs ? 'Tarea'    : 'Task';
  String get legendObjective => isEs ? 'Objetivo' : 'Objective';
  String get legendEvent     => isEs ? 'Evento'   : 'Event';

  // Day detail
  String get myEvents        => isEs ? '📅 MIS EVENTOS'   : '📅 MY EVENTS';
  String get addEvent        => isEs ? 'EVENTO'            : 'EVENT';
  String get noEventsDay     => isEs ? 'Sin eventos este día' : 'No events this day';
  String get sectionGoals    => isEs ? '🎯 METAS'          : '🎯 GOALS';
  String get sectionTasks    => isEs ? '📋 TAREAS'         : '📋 TASKS';
  String get sectionObjs     => isEs ? '✅ OBJETIVOS'      : '✅ OBJECTIVES';
  String get deleteEvent     => isEs ? 'Eliminar este evento'  : 'Delete this event';
  String get deleteSeries    => isEs ? 'Eliminar toda la serie' : 'Delete entire series';
  ...
  String get repeatLabel     => isEs ? 'REPETICIÓN'            : 'REPEAT';
  String get daysLabel       => isEs ? 'DÍAS'                  : 'DAYS';
  String get neverEnds       => isEs ? 'NUNCA TERMINA'         : 'NEVER ENDS';
  String get endDateLabel    => isEs ? 'FECHA FIN'             : 'END DATE';
  String get noEndDate       => isEs ? 'SIN FECHA FIN'         : 'NO END DATE';
...
class CalendarEvent {
  ...
  Color get flutterColor => const Color(0xFF5B9BD5);
}
...
class _HomeScreenState extends ConsumerState<HomeScreen>
  ...
  void _updateLevelDerived(int level) {
    ...
    _levelTitle = s.levelTitle(level);
  }
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setDlg) => AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 4),
          Text(s.freezeAvailable(freezes),
  ...
  Widget build(BuildContext context) {
    ...
    final s           = _HomeS(lang == AppLanguage.es);
    final habitsState = ref.watch(habitsProvider);
    final freezes     = habitsState.freezes;
    final maxStreak   = habitsState.streakData.isEmpty ? 0
        : habitsState.streakData.map((st) => st.streak).reduce((a, b) => a > b ? a : b);
    ...
    // Actualizar levelTitle cuando el idioma cambia
    _levelTitle = s.levelTitle(_level);
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                _buildSectionLabel(context, s.playerStatus),
                const SizedBox(height: 10),
                _buildStatsGrid(context, freezes, maxStreak, s),
                const SizedBox(height: 16),
                _buildSectionLabel(context, s.upcomingDue),
                const SizedBox(height: 10),
                _buildDeadlineCard(context, s),
                const SizedBox(height: 16),
                _buildSectionLabel(context, s.calendar),
                const SizedBox(height: 10),
                _buildCalendar(context, s),
  ...
  Widget _buildCalendar(BuildContext context, _HomeS s) {
    final c          = context.ac;
      ...
      child: Column(children: [
        ...
        Padding(
          ...
          child: Wrap(spacing: 10, runSpacing: 4, alignment: WrapAlignment.center, children: [
            _legendDot(AutumnColors.accentOrange, s.legendGoal),
            _legendDot(AutumnColors.accentGold,   s.legendTask),
            _legendDot(AutumnColors.mossGreen,     s.legendObjective),
            _legendDot(const Color(0xFF5B9BD5),    s.legendEvent),
          ]),
        ),
      ]),
    );
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start, children: [
                      ...
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          child: Row(mainAxisSize: MainAxisSize.min, children: [
                            ...
                            const SizedBox(width: 4),
                            Text(s.addEvent, style: GoogleFonts.pressStart2p(
                            ...
                            PopupMenuButton<String>(
                              ...
                              onSelected: (val) async {
                                if (val == 'single') {
                                  ...
                                  setSheet(() { dayOwnEvents.removeWhere((ev) => ev.id == e.id); });
                                } else if (val == 'series' && e.repeatGroupId != null) {
                              ...
                              itemBuilder: (_) => [
                                PopupMenuItem(value: 'single',
                                    child: Text(s.deleteEvent,
                                        style: GoogleFonts.pressStart2p(fontSize: 8, color: AutumnColors.accentRed))),
                                if (e.repeatGroupId != null)
                                  PopupMenuItem(value: 'series',
                                      child: Text(s.deleteSeries,
                                          style: GoogleFonts.pressStart2p(fontSize: 8, color: AutumnColors.accentRed))),
                              ],
                            ),
                          ]),
                    ...
                    if (goals.isEmpty && todos.isEmpty && objs.isEmpty && dayOwnEvents.isEmpty)
                      Padding(padding: const EdgeInsets.only(bottom: 8),
                          child: Text(s.noEventsDay,
                              style: GoogleFonts.pressStart2p(fontSize: 8, color: c.textDisabled))),
                    if (goals.isNotEmpty) ...[
                      _dayDetailSection(ctx, s.sectionGoals, goals, AutumnColors.accentOrange),
                      const SizedBox(height: 10),
                    ],
                    if (todos.isNotEmpty) ...[
                      _dayDetailSection(ctx, s.sectionTasks, todos, AutumnColors.accentGold),
                      const SizedBox(height: 10),
                    ],
                    if (objs.isNotEmpty)
                      _dayDetailSection(ctx, s.sectionObjs, objs, AutumnColors.mossGreen),
                  ]),
            ),
          ),
        ),
            ...
            child: Column(children: [
              Container(
                ...
                child: Row(children: [
                  Container(width: 10, height: 10,
                      decoration: const BoxDecoration(color: eventColor, shape: BoxShape.circle)),
                  const SizedBox(width: 10),
                  Text(s.newEvent, style: GoogleFonts.pressStart2p(fontSize: 10, color: eventColor)),
                ]),
              ),
              ...
              Expanded(child: SingleChildScrollView(
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Text(s.nameLabel, style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textDisabled)),
                  const SizedBox(height: 6),
                  TextField(
                  ...
                  Text(s.categoryLabel, style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textDisabled)),
                  const SizedBox(height: 6),
                  _buildCategorySelector(
                    c: c, selectedCategory: selectedCategory, accentColor: eventColor,
                  ...
                  Text(s.notesLabel, style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textDisabled)),
                  const SizedBox(height: 6),
                  TextField(
                    controller: notesCtrl, maxLines: 2,
                    style: GoogleFonts.pressStart2p(color: c.textPrimary, fontSize: 9),
                    decoration: InputDecoration(
                      hintText: s.notesPlaceholder,
                      hintStyle: GoogleFonts.pressStart2p(fontSize: 8, color: c.textDisabled),
                      filled: true, fillColor: c.bgSurface,
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(10),
                          borderSide: BorderSide(color: c.divider)),
                  ...
                  GestureDetector(
                    onTap: () => setDlg(() {
                      ...
                      if (!repeatEnabled) { selectedWeekdays.clear(); repeatUntil = null; neverEnds = false; }
                    }),
                    child: Container(
                      padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                      decoration: BoxDecoration(
                          color: repeatEnabled ? eventColor.withValues(alpha: 0.1) : c.bgSurface,
                          borderRadius: BorderRadius.circular(10),
                          border: Border.all(color: repeatEnabled ? eventColor : c.divider,
                              width: repeatEnabled ? 1.5 : 1)),
                      child: Row(children: [
                        Icon(Icons.repeat_rounded, size: 14,
                            color: repeatEnabled ? eventColor : c.textDisabled),
                        const SizedBox(width: 10),
                        Text(s.repeatLabel, style: GoogleFonts.pressStart2p(fontSize: 9,
                            color: repeatEnabled ? eventColor : c.textDisabled)),
                        const Spacer(),
                        Icon(repeatEnabled ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down,
                      ...
                      child: Row(children: [
                        AnimatedContainer(duration: const Duration(milliseconds: 150),
                            ...
                            decoration: BoxDecoration(
                                ...
                                border: Border.all(color: neverEnds ? eventColor : c.divider, width: 1.5)),
                            child: neverEnds ? const Icon(Icons.check, color: Colors.white, size: 12) : null),
                        const SizedBox(width: 10),
                        Text(s.neverEnds, style: GoogleFonts.pressStart2p(fontSize: 8, color: c.textPrimary)),
                      ]),
                    ),

                    if (!neverEnds) ...[
                      const SizedBox(height: 10),
                      Text(s.endDateLabel, style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textDisabled)),
                      const SizedBox(height: 6),
                      GestureDetector(
                        onTap: () async {
                          final picked = await showDatePicker(
                            context: ctx,
                            initialDate: DateTime.parse(date).add(const Duration(days: 7)),
                            ...
                            builder: (dCtx, child) => Theme(
                              data: ThemeData.light().copyWith(colorScheme: const ColorScheme.light(
                                  primary: eventColor, onPrimary: Colors.white,
                                  surface: AutumnColors.bgCard, onSurface: AutumnColors.textPrimary)),
                              child: child!,
                            ),
                          );
                          if (picked != null) setDlg(() => repeatUntil = picked);
                        },
                        child: Container(
                          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 12),
                          decoration: BoxDecoration(color: c.bgSurface, borderRadius: BorderRadius.circular(10),
                              border: Border.all(color: repeatUntil != null ? eventColor : c.divider,
                                  width: repeatUntil != null ? 1.5 : 1)),
                          child: Row(children: [
                            Icon(Icons.calendar_today_rounded, size: 14,
                                color: repeatUntil != null ? eventColor : c.textDisabled),
                            const SizedBox(width: 10),
                            Text(repeatUntil != null
                                ? DateFormat('dd MMM yyyy', 'es').format(repeatUntil!)
                                : s.noEndDate,
                                style: GoogleFonts.pressStart2p(fontSize: 9,
                                    color: repeatUntil != null ? c.textPrimary : c.textDisabled)),
                            const Spacer(),
                            if (repeatUntil != null)
                              GestureDetector(onTap: () => setDlg(() => repeatUntil = null),
                                  child: Icon(Icons.close, size: 14, color: c.textDisabled)),
                          ]),
                        ),
                      ),
  ...
  Widget _buildCategorySelector({
    ...
    return Autocomplete<String>(
      optionsBuilder: (tv) {
        ...
        if (q.isEmpty) return categories.map((cat) => '${cat['emoji']} ${cat['label']}');
        return categories.where((cat) => cat['label']!.toLowerCase().contains(q))
      ...
      fieldViewBuilder: (ctx2, ctrl2, fn, _) => TextField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: selectedCategory != null
              ...
              child: Text(categories.firstWhere(
          ...
          suffixIcon: selectedCategory != null
              ? Icon(Icons.check_circle, size: 16, color: accentColor) : null,
          filled: true, fillColor: c.bgSurface,
          border: OutlineInputBorder(borderRadius: BorderRadius.circular(10),
              borderSide: BorderSide(color: c.divider)),
          focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(10),
  ...
  Widget _buildTreeHero(BuildContext context, double xpProgress, _HomeS s) {
    final c = context.ac;
    return Container(
      decoration: BoxDecoration(
        ...
        SizedBox(height: 200, child: Stack(alignment: Alignment.center, children: [
          Positioned(bottom: 0, left: 0, right: 0, child: Container(height: 32,
              decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter,
                    ...
                    AutumnColors.mossGreen.withValues(alpha: 0),
                    AutumnColors.mossGreen.withValues(alpha: 0.08)])))),
  ...
  Widget _buildStatsGrid(BuildContext context, int freezes, int maxStreak, _HomeS s) {
    final stats = [
      (freezes,     '❄',  'FREEZES',   AutumnColors.freeze),
      (_goalsCount, '🎯', s.statGoals,  AutumnColors.accentOrange),
      (maxStreak,   '🔥', s.statStreak, AutumnColors.accentGold),
      (_todosCount, '📋', s.statTasks,  AutumnColors.mossGreen),
    ];
    return GridView.count(crossAxisCount: 4, shrinkWrap: true,
        physics: const NeverScrollableScrollPhysics(),
        crossAxisSpacing: 8, mainAxisSpacing: 8, childAspectRatio: 0.82,
        children: stats.map((st) => _statCard(context, st.$1.toString(), st.$2, st.$3, st.$4)).toList());
  }
  ...
  Widget _buildDeadlineCard(BuildContext context, _HomeS s) {
    final c = context.ac;
    return Container(
