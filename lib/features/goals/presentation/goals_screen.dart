...
final _categoryEmojiMap = {for (final c in kAppCategories) c['label']!: c['emoji']!};
...
class _GoalsScreenState extends ConsumerState<GoalsScreen>
  ...
  Widget _dlgInput(BuildContext ctx, TextEditingController ctrl, String hint, {bool multiline = false}) {
    final c = ctx.ac;
    return TextField(
        controller: ctrl, maxLines: multiline ? 3 : 1,
  ...
  Widget _fLabel(BuildContext ctx, String t) =>
      Text(t, style: GoogleFonts.pressStart2p(fontSize: 7, color: ctx.ac.textDisabled));
  ...
  void _openCreateGoalPopup() {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setDlg) => Dialog(
        ...
        child: SizedBox(width: 400, height: MediaQuery.of(ctx).size.height * 0.85,
          child: Column(children: [
            ...
            Expanded(child: SingleChildScrollView(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(color: c.bgSurface, borderRadius: BorderRadius.circular(10),
                        border: Border.all(color: selectedDate != null ? AutumnColors.accentOrange : c.divider,
                    ...
                    child: Row(children: [
                      ...
                      if (selectedDate != null)
                        Container(
                            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                            decoration: BoxDecoration(color: AutumnColors.accentOrange.withOpacity(0.15), borderRadius: BorderRadius.circular(6)),
                ...
                _buildCategoryAutocomplete(ctx, categoryCtrl: categoryCtrl,
                ...
                Row(children: ['Alta', 'Moderada', 'Baja'].map((p) {
                  ...
                  return Expanded(child: GestureDetector(
                      ...
                      child: AnimatedContainer(duration: const Duration(milliseconds: 150),
                          ...
                          decoration: BoxDecoration(color: sel ? pc[p] : c.bgSurface,
                ...
                NotificationConfigWidget(config: notifConfig, onChanged: (cfg) => setDlg(() => notifConfig = cfg)),
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      final newId = await ref.read(goalsProvider.notifier).createGoal(
                          title: t2, description: descCtrl.text.trim(),
                    ...
                    child: Text('✅ INICIAR MISIÓN', style: GoogleFonts.pressStart2p(fontSize: 9, color: c.bgCard)),
                  ),
                ])),
          ]),
        ),
        ...
        Expanded(child: Padding(padding: const EdgeInsets.fromLTRB(14, 12, 12, 12),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            if (goal.description.isNotEmpty) ...[
              Text(goal.description, style: GoogleFonts.pressStart2p(fontSize: 8, color: c.textSecondary),
            ...
            Row(children: [
              ...
              if (deadlineBadge.isNotEmpty)
                Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                    decoration: BoxDecoration(color: deadlineColor.withOpacity(0.12), borderRadius: BorderRadius.circular(8),
            ...
            if (goal.objTotal > 0) ...[
              ...
              Stack(children: [
                ...
                FractionallySizedBox(widthFactor: pct, child: Container(height: 14,
                    decoration: BoxDecoration(
                        gradient: LinearGradient(colors: [AutumnColors.accentOrange, pct >= 1.0 ? AutumnColors.mossGreen : AutumnColors.accentGold]),
                        borderRadius: BorderRadius.circular(7),
                        boxShadow: pct > 0 ? [BoxShadow(color: AutumnColors.accentOrange.withOpacity(0.35), blurRadius: 4)] : []))),
                if (pct > 0.18)
                  Positioned.fill(child: Center(child: Text('$pctInt%',
              ...
              Row(children: [
                ...
                const SizedBox(width: 6),
                Text('Sin objetivos — toca para añadir', style: GoogleFonts.pressStart2p(fontSize: 7, color: c.textDisabled)),
              ]),
              const SizedBox(height: 8),
            ],
            Row(children: [
  ...
  void _openObjectivesPopup(Goal goal) {
    if (!mounted) return;
    showDialog(context: context, builder: (ctx) => _ObjectivesDialog(
        goal: goal, userId: widget.userId,
        onChanged: (total, completed) =>
            ref.read(goalsProvider.notifier).updateGoalObjStats(goal.id, total, completed)));
  }
          ...
          child: _filtersOpen
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(children: ['TODAS', 'Alta', 'Moderada', 'Baja'].map((p) {
                  ...
                  const colors = {'Alta': AutumnColors.accentRed, 'Moderada': AutumnColors.accentGold, 'Baja': AutumnColors.mossGreen, 'TODAS': AutumnColors.accentOrange};
          ...
          if (_message.isNotEmpty)
            Container(margin: const EdgeInsets.only(bottom: 8),
                ...
                decoration: BoxDecoration(color: _messageColor.withOpacity(0.12),
                    borderRadius: BorderRadius.circular(8), border: Border.all(color: _messageColor.withOpacity(0.3))),
                child: Text(_message, style: GoogleFonts.pressStart2p(fontSize: 9, color: _messageColor), textAlign: TextAlign.center)),
...
class _ObjectivesDialogState extends State<_ObjectivesDialog> {
  ...
  @override void initState() { super.initState(); _loadObjectives(); }
  ...
  void _notifyParent() => widget.onChanged(_objectives.length, _objectives.where((o) => o.isCompleted).length);
    ...
    try {
      ...
      if (newStatus == 'completed') {
        ...
        final existing = await _db.from('xp_log').select('id')
            .eq('source', 'objective_completed').eq('source_id', objId).eq('event_date', today).maybeSingle();
        if (existing == null) {
  ...
  Widget build(BuildContext context) {
    ...
    return Dialog(
      backgroundColor: c.bgCard,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
      child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 440, maxHeight: 620),
        child: Column(children: [
          Container(padding: const EdgeInsets.all(16),
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                if (total > 0) ...[
                  ...
                  Stack(children: [
                    ...
                    AnimatedFractionallySizedBox(duration: const Duration(milliseconds: 400), widthFactor: progress,
                        child: Container(height: 14, decoration: BoxDecoration(
                            gradient: LinearGradient(colors: [AutumnColors.accentOrange, progress >= 1.0 ? AutumnColors.mossGreen : AutumnColors.accentGold]),
                            borderRadius: BorderRadius.circular(7),
                  ...
                  if (progress >= 1.0) Padding(padding: const EdgeInsets.only(top: 6),
                  ...
                  GestureDetector(
                    ...
                    child: AnimatedContainer(duration: const Duration(milliseconds: 200),
                        ...
                        child: Center(child: obj.isCompleted
                            ...
                            : obj.isHabit ? const Icon(Icons.link, color: AutumnColors.accentGold, size: 16) : null)),
                  ),
                  const SizedBox(width: 10),
  ...
  Future<void> _addObjective() async {
    ...
    try {
      ...
      final result  = await _db.from('objectives').insert({
        'goal_id': widget.goalId, 'title': _titleCtrl.text.trim(),
        'description': '', 'deadline': _deadline?.toIso8601String().substring(0, 10) ?? '',
        'type': habitId != null ? 'habit' : 'manual', 'habit_id': habitId, 'status': 'pending',
      }).select('id').single();
      if (_notifConfig.enabled && _deadline != null) {
        try {
          await NotificationService().scheduleDeadlineNotifications(
              itemId: result['id'] as int, itemType: 'objective',
              title: _titleCtrl.text.trim(), deadline: _deadline!, config: _notifConfig.toConfigMap());
        } catch (_) {}
      }
      _titleCtrl.clear();
      setState(() { _deadline = null; _selectedHabitId = null; _linkHabit = false; _notifConfig = const NotificationConfig(); });
      widget.onAdded();
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [
        Expanded(child: TextField(controller: _titleCtrl,
            ...
            decoration: InputDecoration(hintText: 'Nuevo objetivo...',
                ...
                focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(8), borderSide: const BorderSide(color: AutumnColors.accentOrange)),
        ...
        Expanded(child: GestureDetector(
          onTap: () async {
            final p = await showDatePicker(context: context,
                ...
                builder: (ctx, child) => Theme(data: ThemeData.light().copyWith(
                    colorScheme: const ColorScheme.light(primary: AutumnColors.accentOrange)), child: child!));
          ...
          child: Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 7),
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.calendar_today, size: 12, color: _deadline != null ? AutumnColors.accentOrange : c.textDisabled),
                const SizedBox(width: 6),
      ...
      if (_linkHabit && _habits.isNotEmpty) ...[
        ...
        Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 2),
            decoration: BoxDecoration(color: c.bgCard, borderRadius: BorderRadius.circular(8), border: Border.all(color: AutumnColors.accentGold)),
            child: DropdownButtonHideUnderline(child: DropdownButton<int>(
                isExpanded: true, value: _selectedHabitId,
                hint: Text('Selecciona un hábito...', style: GoogleFonts.pressStart2p(fontSize: 8, color: c.textDisabled)),
